 ...
 * @param {type} type a type from Types, if not given, reader will read qvariant
 * @example
...
Reader.prototype.parse = function(){
    if (this.type) {
        this.parsed = this.getQVariantByType(this.type);
    } else {
        this.parsed = this.getQVariant();
    }
    if (debug && this.pos !== this.size + 4) {
        logger("packet incorrectly sized (" + this.pos + "/" + this.size);
    }
    return this.parsed;
};
...
Reader.prototype.getMap = function(){
    var mapSize = this.getUInt(), i, map = Object.create(null), key, val;
    for (i=0; i<mapSize; i++) {
        // get key
        key = this.getString();
        // get value
