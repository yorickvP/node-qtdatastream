...
Writer.prototype._parse_qusertype = function(obj){
    ...
    if (qtdatastream.isUserTypeComplex(name)) {
        qtdatastream.getUserType(name).forEach((item) => {
            const key = Object.keys(item), val = item[key];
            const typ = obj.obj[key];
            if (typeof val === 'string') {
                if (typ instanceof QUserType) {
                    return this._parse(typ);
                } else {
                    return this._parse(new QUserType(val, typ));
                }
            } else {
                return this._parse(qtdatastream.Class(val, typ));
            }
        });
    } else {
